...
namespace PokemonStrategyBuilder.Domain.Services;

public class TypeEffectivenessService : ITypeEffectivenessService
{
    private static readonly Dictionary<(PokemonType Attacking, PokemonType Defending), double> EffectivenessChart = new()
    {
        // Normal
    ...
    {
        return EffectivenessChart.TryGetValue((attackingType, defendingType), out var multiplier)
            ? multiplier
            : 1.0;
    }
    ...
    {
        var primaryMultiplier = GetEffectiveness(attackingType, defendingPrimaryType);

        if (!defendingSecondaryType.HasValue)
        {
            return primaryMultiplier;
        }

        var secondaryMultiplier = GetEffectiveness(attackingType, defendingSecondaryType.Value);
